...
import { BusinessType, Tone, Purpose, User, GeneratedPost } from '../types';
import { generateInstaContent } from '../geminiService';
...
const CreatePostPage: React.FC<CreatePostPageProps> = ({ onGenerate, user }) => {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const result = await generateInstaContent(businessType, content, tone, purpose);
          ...
          <div className="bg-white dark:bg-surface-dark rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-800 space-y-8">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button 
                  ...
                  className={`p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${businessType === BusinessType.CAFE ? 'border-primary bg-primary/5 text-primary' : 'border-gray-100 text-gray-400'}`}
                >
                ...
                <button 
                  ...
                  className={`p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all ${businessType === BusinessType.SALON ? 'border-primary bg-primary/5 text-primary' : 'border-gray-100 text-gray-400'}`}
                >
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full h-32 rounded-xl border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-background-dark focus:ring-primary focus:border-primary resize-none"
              />
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button 
                    ...
                    className={`p-3 rounded-lg border flex flex-col items-center text-xs transition-all ${tone === t.id ? 'border-primary bg-primary/5 text-primary font-bold' : 'border-gray-100 text-gray-500'}`}
                  >
            ...
            <button 
              ...
              disabled={loading}
              className="w-full py-4 rounded-2xl bg-primary hover:bg-primary-hover text-white font-bold text-lg shadow-lg shadow-primary/20 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
